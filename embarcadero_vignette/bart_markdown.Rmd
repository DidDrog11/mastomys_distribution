...
load(file = here("embarcadero_vignette", "data", "covsraw.rda"))
covs <- covsraw
class(covs)
...
load(file = here("embarcadero_vignette", "data", "ticks.rda"))
head(ticks)
nrow(ticks)
```
...
pres.cov <- terra::extract(covs, pts.sp1[, 1:2])
head(pres.cov)
```
...
load(file = here("embarcadero_vignette", "data", "cchf.rda"))
head(cchf)
nrow(cchf)
...
sp.bio12 <- spartial(cchf.model, covs, x.vars = 'bio12', equal = TRUE)
plot(sp.bio12, 
     box = FALSE,
     axes = FALSE,
     main = 'Spartial plot: bio12',
     #zlim = c(0,1),
     #axis.args=list(at=pretty(0:1), labels=pretty(0:1)),
